...
\cls{ltx-talk}, which takes ideas from \cls{beamer} but with tagging and
...
expectation for \cls{ltx-talk} is that users \emph{will} need to change their
sources. Unlike other documents, presentations tend to be \enquote{single use}:
revised and adjusted each time they are used. The need to edit sources should
therefore not be an insurmountable barrier.
...
throghout. Over time, the aim is to address many of the things that
...
Currently, \cls{ltx-talk} \emph{absolutely requires} the use of the
\cs{DocumentMetadata} command \emph{and} the a setting for the \texttt{tagging}
key. As such, the most basic \cls{ltx-talk} document is
\begin{verbatim}
  \DocumentMetadata{tagging = on} % or "tagging = draft"/"tagging = off"
...
\opt{mode} option set to \meta{hanout}. This will cause the document to be
