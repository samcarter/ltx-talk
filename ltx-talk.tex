...
\def\filedate{2026-09-26}
\def\fileversion{v0.6.7}

\begin{document}

\title{%
  \cls{ltx-talk} -- A class for typesetting (accessible) presentations%
...
At present, settings for \cls{ltx-talk} are not all controlled from a single
location. Over time this is likely to evolve and be more clearly documented.
Where keys are used for \foreign{ad hoc} settings, these can typically be set
globally by using \cs{SetKeys}\texttt{[talk]\{\meta{keys}\}}. The keys are
currently in different subtrees. For example, overall frame vertical alignment
could be set using
\begin{verbatim}
  \SetKeys[talk]{frame/vertical-alignment = top}
\end{verbatim}
Work on both the keys themselves and the documentation is on the active list of
tasks to complete.
...
The \env{frame} and \env{frame*} environments recognise the following options
\begin{itemize}
  \item \opt{action-spec} Applies the \opt{action-spec} to all nested
    environments which take a suitable specification: see
    Section~\ref{sec:lists} for more details.
  \item \opt{auto-break} and \item \opt{auto-break-coverage} Control automatic
    breaking of frames into slides: see Section~\ref{sec:auto-break} for more
    details.
  \item \opt{name} Sets the name of a frame for reuse: see
    Section~\ref{sec:reusing}.
  \item \opt{label} An alternative for \opt{name}, available for compatibility
    with \cls{beamer}.
  \item \opt{supplementary-frame} Indicates the frame is supplementary: see
    Section~\ref{sec:suppl}.
  \item \opt{vertical-alignment} Defines the vertical alignment of frame
    content: choice of \opt{bottom}, \opt{center}, \opt{top} or \opt{stretch}.
\end{itemize}

For compatibility with \cls{beamer}, the optional argument will allow the use
of an \opt{action-spec} without a key name, for example
\begin{verbatim}
  \begin{frame}[<+->]
\end{verbatim}
As a result, at least one |=| must appear in the optional argument for
\cls{ltx-talk} to interpret it as a key--value list. This means that if the
\opt{auto-break} option is given with no others, you \emph{must} includes
|= true|.
...
\end{verbatim}
To adjust the transparency, use
\begin{alltt}
  \cs{EditInstance}\{hidden\}\{std\}\{opacity = \meta{value}\}
\end{alltt}
where \meta{value} ranges from~$0$ (totally invisible) to $1$ (completely
visible).
...
\DescribeEnv{block}
\begin{syntax}
  \cs{begin}\{\texttt{block}\}\sarg{overlay spec}\marg{block title}
\end{syntax}
Inserts a block, like a definition or a theorem, with the title \meta{block
title}. If the \meta{action specification} is present, the given actions are
taken on the specified slides, see Section~\ref{sec:action-spec}. In the
example, the definition is shown only from slide~3 onward.
\begin{verbatim}
...
\subsection{Supplementary frames%
  \label{sec:suppl}}

Sometimes, more frames are available than are needed for the presentation. One
approach to handling these supplementary frames is to collect them after the
\cs{appendix} command in the source, so that they appear at the end of the
presentation. This may mean they are out of logical order, and also means it is
more difficult to provide both a \enquote{short} and \enquote{long} version of
the same content.

To address this, \cls{ltx-talk} provides the class option
\opt{supplementary-frames} and frame key \opt{supplementary-frame}. Frames
marked as as supplementary then have three different handling possibilities,
set using the class option:
\begin{itemize}
  \item \opt{appendix}: Move to the appendix automatically (after any explicit
    appendix)
  \item \opt{in-place}: Output in place in the source, \foreign{i.e.}~the
    same as if the frames where not marked as supplementary
  \item \opt{omit}: Simply omit the the frames entirely
\end{itemize}

When frames are automatically moved to the appendix, they are names
\texttt{appendix.\meta{n}}, where \meta{n} is a serial number; these therefore
can be accessed using \cs{reuseframe}.

\subsection{Automatic frame breaking%
  \label{sec:auto-break}}

In the \emph{vast} majority of cases, frames should be split by the author when
there is too much content to appear in one go. However, in the case of
automatically-generated material, most commonly bibliographies, this may not be
feasible. To allow for this, the frame option \opt{auto-break} is available.

When auto-breaking is activated, \cls{ltx-talk} will split the content of the
frame into different slides based on the vertical height. The split will be
activated if the natural height of the content is greater than \cs{textheight},
and each slide with use \opt{auto-break-coverage} percent of the
\cs{textheight} for the split.

Note that overlays are disabled when breaking is active, and that formal
support here is specifically targetded at auto-generated content.
...
At present, \cls{ltx-talk} loads the \pkg{fontspec} package when used with
suitable engines (Xe\TeX{} and Lau\TeX{}).\footnote{This is skipped for some
very specialized development cases: you know if this applies to you!} This means
that the interfaces \cs{setmainfont} and \cs{setsansfont} can be used to change
the body font. This \emph{likely will change} in time: whilst \pkg{fontspec} is
convenient for users, there are technical issues that mean that alternatives are
being examined. As such, we \emph{strongly} recommend that you load
\pkg{fontspec} explicitly if you make use of its functionality.
...
they have specific font requirements. If you want to set the math font
directly, you may also need to adjust the one use for \cs{mathrm}, for example
\begin{verbatim}
...
\begin{itemize}
  \item \texttt{appendixframenumber}
  \item \texttt{appendixframes}
  \item \texttt{author}
  \item \texttt{date}
...
\paragraph{Adding an appendix}

You can add an appendix to your talk by using the \cs{appendix} command. You
should put frames and perhaps whole subsections into the appendix that you do
not intend to show during your presentation, but which might be useful to answer
a question. The \cs{appendix} command essentially just starts a new part.

\DescribeMacro{\appendix}
\begin{syntax}
  \cs{appendix}\sarg{mode spec}
\end{syntax}
Starts the appendix in the specified modes. All frames, all \cs{subsection}
commands, and all \cs{section} commands used after this command will not be
shown as part of the normal navigation bars.
\begin{verbatim}
  \begin{document}
  \begin{frame}
    \maketitle
...
Some interfaces for customization are provided using the L3 programming layer;
these are intended for programmers, and are currently somewhat experimental.

\begin{variable}{\l_talk_frame_properties_clist}
  Contains names of properties which should be recorded at the end of
  each frame. The properties should be defined using \cs{property_new:nnnn}.
  The class does not add any entries to this list.
\end{variable}
