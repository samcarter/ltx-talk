...
\def\filedate{2025-07-28}
\def\fileversion{v0.1.5}
...
process occurs to that for |<+->|, \emph{except} that the counter |pauses| used
unchanged. Thus a dot, possibly followed by an offset, just expands to the
...
\cs{date}, \cls{ltx-talk} supports \cs{subtitle} and \cs{institute}. These work
in the same way as the standard commands for providing metadata. The
\meta{options} argument is currently unused.
