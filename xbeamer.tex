...
\DescribeEnv{columns}
\begin{syntax}
  \cs{begin}\{columns\}\sarg{action spec}\oarg{options}
\end{syntax}
A multi-column area. If the \meta{action spec} is present, the given actions
are taken on the specified slides, see
Section~\ref{section-action-specifications}.

\DescribeEnv{column}
\begin{syntax}
  \cs{begin}\{column\}\sarg{action spec}\oarg{placement}\marg{column width}
\end{syntax}
...
see Section~\ref{section-action-specifications}. The vertical placement of the
enclosing \env{columns} environment can be overruled by specifying a specific
\meta{placement}.
