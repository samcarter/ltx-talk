...
\DescribeOption{mode}
The \cls{xbeamer} class allows you to select different output formats from the
...
As detailed in the introduction (Section~\ref{sec:intro}), the \cls{xbeamer}
...
\opt{tag-slides} option, for example \opt{tag-slides = n|handout:1,n} would tag
the last slide in the frame in \texttt{projector} mode and the first and last
for \texttt{handout} mode.
